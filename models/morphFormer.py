...
class CNN(nn.Module):
    def __init__(self, FM, band, Classes, patch_size, HSIOnly=True):
        ...
        self.conv6 = nn.Sequential(
            HetConv(8 * (band - 8), FM * 4,
                    p = 1,
                    g = (FM*4)//4 if (8 * (band - 8)) % FM == 0 else (FM * 4) // 8,
                    ),
            nn.BatchNorm2d(FM*4),
            nn.ReLU()
        )
...
def morphFormer(dataset, patch_size):
    model = None
    if dataset == 'sa':
        model = CNN(16, band=204, Classes=16, patch_size=patch_size)

    elif dataset == 'pu':
        model = CNN(16, band=103, Classes=9, patch_size=patch_size)

    elif dataset == 'whulk':
        model = CNN(16, band=270, Classes=9, patch_size=patch_size)

    elif dataset == 'hrl':
        model = CNN(16, band=176, Classes=14, patch_size=patch_size)

    elif dataset == 'IP':
        model = CNN(16, band=200, Classes=16, patch_size=patch_size)

    elif dataset == 'whuhc':
        model = CNN(16, band=274, Classes=16, patch_size=patch_size)

    elif dataset == 'pc':
        model = CNN(16, band=102, Classes=9, patch_size=patch_size)

    elif dataset == 'whuhh':
        model = CNN(16, band=270, Classes=22, patch_size=patch_size)
...
if __name__ == "__main__":
    ...
    t = torch.randn(size=(3, 1, 200, 11, 11))
    print("input shape:", t.shape)
    net = morphFormer(dataset='IP', patch_size=11)
