...
class DCTN(nn.Module):
    ...
    def __init__(self, layers, img_size=15, patch_size=3, in_chans=3, num_classes=1000,
                 ...
                 norm_layer=nn.LayerNorm, mlp_fn=EISA, dataset="pu"):

        super().__init__()
        self.num_classes = num_classes
        self.patch_embed = MDCP(img_size=img_size, patch_size=patch_size, in_chans=in_chans,
                                      embed_dim=embed_dims[0], dataset=dataset)
        network = []
...
if __name__ == "__main__":
    ...
    t = torch.randn(size=(3, 1, 200, 7, 7))
    print("input shape:", t.shape)
    net = DCTN(layers=1, dataset='IP', img_size=7, num_classes=9)
    ...
    mlp_ratios = [3, 3, 3, 3]
    if dataset == "pu":
        embed_dims = [320, 320, 512, 512]
    elif dataset == "IP":
        embed_dims = [440, 440, 512, 512]
    elif dataset == "HsU":
        embed_dims = [320, 320, 512, 512]
    else:
        embed_dims = [168, 168, 512, 512]
    layers = [2, 2, 5, 3]
    # model = DCTN(layers, img_size=15, in_chans=n_bands, num_classes=n_classes, embed_dims=embed_dims, patch_size=3,
    #              transitions=transitions, segment_dim=segment_dim, dateset=dataset, )