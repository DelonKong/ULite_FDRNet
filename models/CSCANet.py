...
class overall(nn.Module):
    def __init__(self, in_channels=1, num_classes=16, num_tokens=4, dim=64):
    ...
    def forward(self, x):
        ...
        # 动态截取参数至当前批次大小
        current_wAx = self.token_wAx[:batch_size]
        current_wVx = self.token_wVx[:batch_size]
        current_wAy = self.token_wAy[:batch_size]
        current_wVy = self.token_wVy[:batch_size]

        wax = rearrange(current_wAx, 'b h w -> b w h')  # Transpose
        Ax = torch.einsum('bij,bjk->bik', x, wax)
        Ax = rearrange(Ax, 'b h w -> b w h')  # Transpose
        Ax = Ax.softmax(dim=-1)
        VVx = torch.einsum('bij,bjk->bik', x, current_wVx)
        ...
        Ay = Ay.softmax(dim=-1)
        VVy = torch.einsum('bij,bjk->bik', current_wVy, y)
...
def CSCANet(dataset, patch_size, pca=False):
    ...
    if pca:
        model = overall(in_channels=1, num_classes=num_classes, num_tokens=4, dim=64)
    else:
        warnings.warn("Non-PCA mode is not supported and may cause errors in model creation. Returning None model.")
        model = None
    return model


if __name__ == '__main__':
    t = torch.randn(size=(64, 1, 30, 9, 9))
    dataset = 'pu'
    print("input shape:", t.shape)
    net = CSCANet(dataset=dataset, patch_size=9, pca=True)
