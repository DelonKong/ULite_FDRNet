...
class GaussianAdaptiveAttention(nn.Module):
    def __init__(self, norm_axis, num_gaussians, eps=1e-8):
    ...
    def forward(self, x):
        mean = x.mean(dim=self.norm_axis, keepdim=True)
        var = x.var(dim=self.norm_axis, keepdim=True, unbiased=False) + self.eps
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x): # x: (B, H, W, C)
        ...
        x = self.norm(x).permute(0, 2, 3, 1) # (b, oc, oh, ow)
        return x
...
class RelPos2d(nn.Module):
    ...
    def generate_2d_decay(self, H: int, W: int):
        ...
        mask = (mask.abs()).sum(dim=-1)  ## for mahatten distance
    ...
    def generate_1d_decay(self, l: int):
        ...
        mask = index[:, None] - index[None, :] # (l, l)  for mahatten distance
        mask = mask.abs() # (l, l)
...
class MaSAd(nn.Module):
    def __init__(self, embed_dim, num_heads, value_factor=1):
        ...
        self.lepe = DWConv2d(embed_dim, 3, 1, 1)

        # gaussian attention
        self.norm_axis = 0
        self.attention_heads = nn.ModuleList([
            GaussianAdaptiveAttention(norm_axis=-self.norm_axis, num_gaussians=1)
    ...
    def forward(self, x: torch.Tensor, rel_pos, chunkwise_recurrent=False, incremental_state=None):
        ...
        output = output.permute(0, 3, 1, 2, 4) # (b, h, w, n, d2)
        out_list = []
        for i in range(self.num_heads):
            out = self.attention_heads[i](output[:,:,:,i,:])
            out_list.append(out.unsqueeze(3))
        out_final = torch.cat(out_list, dim=3).flatten(-2, -1) # (b, h, w, n*d2)

        output = out_final + lepe
...
class FeedForwardNetwork(nn.Module):
    def __init__(self, embed_dim, ffn_dim, activation_fn=F.gelu, dropout=0.0, activation_dropout=0.0, layernorm_eps=1e-6, subln=False, subconv=False):
...
class BasicLayer(nn.Module):
    ...
    Args:
        ...
        qk_scale (float | None, optional): Dropout rate. Default: 0.0
        attn_drop (float, optional): Attention dropout rate. Default: 0.0
        drop_path (float | tuple[float], optional): Stochastic depth rate. Default: 0.0
        norm_layer (nn.Module, optional): Normalization layer. Default: nn.LayerNorm
        downsample (nn.Module | None, optional): Downsample layer at the end of the layer. Default: None
        use_checkpoint (bool): Whether to use checkpointing to save memory. Default: False
        fused_window_process (bool, optional): If True, use one kernel to fuse window shift & window partition for acceleration,
    ...
    def __init__(self, embed_dim, out_dim, depth, num_heads, init_value: float, heads_range: float,
                 ffn_dim=128., drop_path=0., norm_layer=nn.LayerNorm, chunkwise_recurrent=False,
...
class VisRetNet(nn.Module):
    def __init__(self, in_chans=3, num_classes=16, embed_dims=[32], depths=[1],
                 num_heads=[4], init_values=[2], heads_ranges=[4], mlp_ratios=[2],
                 drop_path_rate=0.1, use_checkpoints=[False], chunkwise_recurrents=[True],
                 layerscales=[False]):
        super(VisRetNet, self).__init__()
        ...
        self.head = nn.Linear(self.num_features*1, self.num_classes) if self.num_classes > 0 else nn.Identity()
