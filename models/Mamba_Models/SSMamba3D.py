...
def SSMamba3D(dataset, patch_size=9, pca=False):
    model = None
    if dataset == 'sa':
        n_bands=204
        num_classes=16
    ...
    model = VisionMamba(
        ...
        k_group=4,
        embed_dim=32,
        dt_rank=2,  # Rank of the dynamic routing tensor
        d_inner=64,  # 256,  # Inner dimension of the model
        d_state=16,  # 256,  # State dimension of the model
...
if __name__ == '__main__':
    ...
    net = SSMamba3D(dataset=dataset).to(device)
