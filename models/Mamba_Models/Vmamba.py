...
class SS2Dv0:
    ...
    def forwardv0(self, x: torch.Tensor, seq=False, force_fp32=True, **kwargs):
        x = self.in_proj(x)  # [b, h, w, d] -> [b, h, w, 2d]
        x, z = x.chunk(2, dim=-1)  # (b, h, w, d) * 2
        z = self.act(z)
        ...
        # 方向融合
        # k=4
        # parallel out_y: B, K, D, L ->out_y[:, 2:4]-> B, 2, D, L
        # seq out_y: B, K*D, L ->out_y[:, 2:4]-> B, 2*D, L-> B, 2, D, L
        inv_y = torch.flip(out_y[:, 2:4], dims=[-1]).view(B, 2, -1, L)
        wh_y = torch.transpose(out_y[:, 1].view(B, -1, W, H), dim0=2, dim1=3).contiguous().view(B, -1, L)
        invwh_y = torch.transpose(inv_y[:, 1].view(B, -1, W, H), dim0=2, dim1=3).contiguous().view(B, -1, L)
        y = out_y[:, 0] + inv_y[:, 0] + wh_y + invwh_y
...
class VSSM(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        x = self.patch_embed(x)  # [bs, c, h, w] -> [bs, h, w, c]
...
if __name__ == '__main__':
    ...
    # t = torch.randn(size=(1, 256, 256, 3)).to(device)
    t = torch.randn(size=(1, 3, 256, 256)).to(device)
    dataset = 'whuhh'
    print("input shape:", t.shape)
    net = vanilla_vmamba_tiny().to(device)
    print("output shape:", net(t).shape)

    with torch.no_grad():
        sum = summary(net, input_size=(1, t.shape[1], t.shape[-2], t.shape[-1]), verbose=0)
        print(sum)
