...
# implementation of DeformROIPool=============================================
import torch
...
class DeformRoIPool(nn.Module):
    def __init__(self, output_size=(7, 7), gamma=0.1):
        super().__init__()
        self.output_size = output_size
        self.gamma = gamma  # 偏移量缩放系数

    def forward(self, x, rois):
        """
        x: 输入特征图 (N, C, H, W)
        rois: ROI区域 (K, 5) [batch_index, x1, y1, x2, y2]
        """
        # 1. 使用标准ROI Align获取初始特征
        pooled = roi_align(
            x,
            rois,
            output_size=self.output_size,
            spatial_scale=1.0,  # 假设特征图已经是原始图像比例
            aligned=True
        )

        # 2. 生成偏移量网格 (简化实现)
        K, C, H, W = pooled.shape
        grid_y, grid_x = torch.meshgrid(
            torch.linspace(-1, 1, H, device=x.device),
            torch.linspace(-1, 1, W, device=x.device),
            indexing='ij'
        )
        grid = torch.stack((grid_x, grid_y), dim=-1).unsqueeze(0).repeat(K, 1, 1, 1)

        # 3. 应用随机偏移 (实际应用中应替换为学习到的偏移量)
        # 这里使用简化随机偏移作为示例
        offset = torch.randn(K, 2, H, W, device=x.device) * self.gamma
...
def hyper_vmamba_overall(in_chans, num_classes, **kwargs):
    model = VSSM(
        depths=[1, ], dims=[96], drop_path_rate=0,
        patch_size=1, in_chans=in_chans, num_classes=num_classes,
        ssm_d_state=16, ssm_ratio=1.0, ssm_dt_rank="auto", ssm_act_layer="silu",
        ssm_conv=3, ssm_conv_bias=True, ssm_drop_rate=0.0,
        ssm_init="v0", forward_type="v2",
        ...
        use_checkpoint=False, posembed=False, imgsize=None,
    )
    return model


def HyperMamba(dataset, patch_size=9, pca=False):
    model = None
    if dataset == 'sa':
        n_bands=204
        num_classes=16
    ...
    model = hyper_vmamba_overall(in_chans=n_bands, num_classes=num_classes, patch_size=patch_size)
...
if __name__ == '__main__':
    ...
    net = HyperMamba(dataset=dataset).to(device)
