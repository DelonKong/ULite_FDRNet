...
class ViT(nn.Module):
    ...
    def forward(self, img):
        if len(img.shape) == 5: img = img.squeeze(1)
        img = self.ournet(img)
        img = self.pool(img)
        img = self.conv4(img)
...
def HybridFormer(dataset, patch_size):
    # https://github.com/zhaolin6/HybridFormer/blob/main/models.py
    model = None
    ps = [3, 5]
    if dataset == 'sa':
        model = ViT(image_size=patch_size, patch_size=ps, num_classes=16, dim=100, depth=4, heads=2,
        mlp_dim=2048, channels=204, dropout=0.2, emb_dropout=0.2)

    elif dataset == 'pu':
        model = ViT(image_size=patch_size, patch_size=ps, num_classes=9, dim=100, depth=4, heads=2,
        ...
        model = ViT(image_size=patch_size, patch_size=ps, num_classes=14, dim=100, depth=4, heads=2,
                    mlp_dim=2048, channels=176, dropout=0.2, emb_dropout=0.2)

    elif dataset == 'IP':
        model = ViT(image_size=patch_size, patch_size=ps, num_classes=16, dim=100, depth=4, heads=2,
                    mlp_dim=2048, channels=200, dropout=0.2, emb_dropout=0.2)

    elif dataset == 'whuhc':
        model = ViT(image_size=patch_size, patch_size=ps, num_classes=16, dim=100, depth=4, heads=2,
                    mlp_dim=2048, channels=274, dropout=0.2, emb_dropout=0.2)

    elif dataset == 'whuhh':
        model = ViT(image_size=patch_size, patch_size=ps, num_classes=22, dim=100, depth=4, heads=2,
                    mlp_dim=2048, channels=270, dropout=0.2, emb_dropout=0.2)

    elif dataset == 'pc':
        model = ViT(image_size=patch_size, patch_size=ps, num_classes=9, dim=100, depth=4, heads=2,
                    mlp_dim=2048, channels=102, dropout=0.2, emb_dropout=0.2)

    elif dataset == 'BS':
        model = ViT(image_size=patch_size, patch_size=ps, num_classes=14, dim=100, depth=4, heads=2,
                    mlp_dim=2048, channels=145, dropout=0.2, emb_dropout=0.2)

    return model


if __name__ == "__main__":
    t = torch.randn(size=(64, 1, 270, 7, 7))
    print("input shape:", t.shape)
    net = HybridFormer(dataset='whuhh', patch_size=7)
