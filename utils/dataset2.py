...
"""
按每个类别固定数量划分训练集、测试集、验证集。
    :param gt: 2D整数数组，-1表示未定义，非负整数表示类别标签。
    :param train_num_per_class: 每个类别用于训练的样本数。
    :param test_num_per_class: 每个类别用于测试的样本数。
...
def sample_gt_fixed(gt, train_num_per_class, test_num_per_class):
    """
    按每个类别固定数量划分训练集、测试集、验证集。
    :param gt: 2D整数数组，-1表示未定义，非负整数表示类别标签。
    :param train_num_per_class: 每个类别用于训练的样本数。
    ...
    :return: train_gt, test_gt, val_gt
    """
    classes = np.unique(gt[gt >= 0])  # 提取所有类别
    train_gt = np.full_like(gt, fill_value=-1)
    test_gt = np.full_like(gt, fill_value=-1)
    val_gt = np.full_like(gt, fill_value=-1)

    for cls in classes:
        indices = np.argwhere(gt == cls)  # 当前类别的所有像素位置
        np.random.shuffle(indices)  # 打乱顺序

        # 按数量划分
        train_indices = indices[:train_num_per_class]
        test_indices = indices[train_num_per_class:train_num_per_class + test_num_per_class]
        val_indices = indices[train_num_per_class + test_num_per_class:]

        # 填充训练集、测试集、验证集
        train_gt[tuple(train_indices.T)] = cls
        test_gt[tuple(test_indices.T)] = cls
        val_gt[tuple(val_indices.T)] = cls
...
if __name__ == "__main__":
    ...
    # 调用 sample_gt_fixed
    train_gt, val_gt, test_gt = sample_gt_fixed(gt, train_num_per_class=20, test_num_per_class=10)
